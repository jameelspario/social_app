...
import com.deificindia.indifun.Utility.IndiRandom;
...
public class EmojiRainLayout extends FrameLayout {
    ...
    public void startDropping() {
        ...
        IndiRandom.setSeed(7);
        mWindowHeight = getWindowHeight();
        Disposable subscription = Observable.interval(mDropFrequency, TimeUnit.MILLISECONDS)
    ...
    private void startDropAnimationForSingleEmoji(final ImageView emoji) {
        final TranslateAnimation translateAnimation = new TranslateAnimation(
                ...
                Animation.RELATIVE_TO_SELF, IndiRandom.floatAround(0, 5),
                Animation.RELATIVE_TO_PARENT, 0,
                Animation.ABSOLUTE, mWindowHeight);
        translateAnimation.setDuration((int)
                (mDropAverageDuration * IndiRandom.floatAround(1, RELATIVE_DROP_DURATION_OFFSET)));
    ...
    private ImageView generateEmoji(Drawable emojiDrawable) {
        ...
        final int width = (int) (EMOJI_STANDARD_SIZE * (1.0 + IndiRandom.positiveGaussian()));
        final int height = (int) (EMOJI_STANDARD_SIZE * (1.0 + IndiRandom.positiveGaussian()));
        final LayoutParams params = new LayoutParams(width, height);
        //params.getPercentLayoutInfo().leftMarginPercent = IndiRandom.floatStandard();
