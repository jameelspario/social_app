...
public class TopFansAdapater extends RecyclerView.Adapter<TopFansAdapater.ViewHolder> {


    Context context;
    List<TopFans> topFans;

    public TopFansAdapater(Context context, List<TopFans> topFans) {
    ...
    @NotNull
    @Override
    public TopFansAdapater.ViewHolder onCreateViewHolder(@NonNull @NotNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(context).inflate(R.layout.fragment_topcontent, parent, false);
        return new ViewHolder(view);
    }

    @Override
    public void onBindViewHolder(@NonNull @NotNull TopFansAdapater.ViewHolder holder, int position) {
         ...
         if(topFans11.getGender()!=null && !topFans11.getGender().isEmpty()) {
